...
public class SpringDataJpaHandsonApplication {
    ...
    public CommandLineRunner demo(CountryService countryService, CountryRepository countryRepository) {
        return args -> {
            ...
            System.out.println("\n--- Query Methods feature (findByPopulationGreaterThan) ---");
            countryRepository.findByPopulationGreaterThan(200000000L)
                    .forEach(System.out::println);

            System.out.println("\n--- HQL Query (findByCapitalHQL) ---");
            countryRepository.findByCapitalHQL("Tokyo")
                    .forEach(System.out::println);

            System.out.println("\n--- Native SQL Query (findLargeCountriesNative) ---");
            countryRepository.findLargeCountriesNative(100000000L)
                    .forEach(System.out::println);
